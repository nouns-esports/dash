import { createAction } from "~/packages/server/plugins/createAction";
import { z } from "zod";
...
export const submitTraits = createAction({
    ...
    check: async ({ user, input }) => {
        ...
        const stats = (await fetch("https://gallery.noundry.wtf/api/artists/stats").then((res) =>
            res.json(),
        )) as Array<{
            address: string;
            traits: number;
        ...
        for (const wallet of wallets) {
            const artist = stats.find(
                (artist) => artist.address.toLowerCase() === wallet.address.toLowerCase(),
            );
