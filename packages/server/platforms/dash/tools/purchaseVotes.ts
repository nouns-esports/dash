...
import { escrows, nounsvitationalVotes, passes, points } from "~/packages/db/schema/public";
...
export const purchaseVotes = createTool({
    ...
    execute: async ({ context, runtimeContext }) => {
        await db.primary.transaction(async (tx) => {
            ...
            if (!community) {
                throw new Error("Community is required to purchase votes");
            }
            ...
            await tx
                .insert(nounsvitationalVotes)
                .values({
                    user: user.id,
                    count: context.count,
                })
                .onConflictDoUpdate({
                    target: [nounsvitationalVotes.user],
                    set: {
                        count: sql`${nounsvitationalVotes.count} + ${context.count}`,
                    },
                });
        });
